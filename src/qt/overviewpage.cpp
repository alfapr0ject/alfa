...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizealfaAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizealfaAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizealfaAmount = strAnonymizealfaAmount.remove(strAnonymizealfaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizealfaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizealfaAmount*COIN) nMaxToAnonymize = nAnonymizealfaAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizealfaAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizealfaAmount));
        strAnonymizealfaAmount = strAnonymizealfaAmount.remove(strAnonymizealfaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizealfaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
