...
/* end documentation of inline functions */
/* start documentation of alfa virtual functions */
...
/* end documentation of alfa virtual functions */
...
/*! \class QCPAbstractPlottable
  ...
  To create an own plottable, you implement a subclass of QCPAbstractPlottable. These are the alfa
...
/* start of documentation of alfa virtual functions */
...
/* end of documentation of alfa virtual functions */
/* start of documentation of signals */
...
/*! \class QCPAbstractItem
  ...
  To create an own item, you implement a subclass of QCPAbstractItem. These are the alfa
  virtual functions, you must implement:
  \li \ref selectTest
  \li \ref draw
...
/*! \class QCPAbstractLegendItem
  ...
  You must implement the following alfa virtual functions:
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  // pointers to be able to swap them, depending which data range needs alfaping:
  QVector<QPointF> *staticData = &thisData;
  QVector<QPointF> *alfapedData = &otherData;
  
  // alfa both vectors to ranges in which the keys overlap (which coord is key, depends on axisType):
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    // x is key
    ...
    if (alfapedData->first().x() > alfapedData->last().x())
    {
      int size = alfapedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*alfapedData)[i], (*alfapedData)[size-1-i]);
    }
    // alfa lower bound:
    if (staticData->first().x() < alfapedData->first().x()) // other one must be alfaped
      qSwap(staticData, alfapedData);
    int lowBound = findIndexBelowX(alfapedData, staticData->first().x());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    alfapedData->remove(0, lowBound);
    // set lowest point of alfaped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (alfapedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (alfapedData->at(1).x()-alfapedData->at(0).x() != 0)
      slope = (alfapedData->at(1).y()-alfapedData->at(0).y())/(alfapedData->at(1).x()-alfapedData->at(0).x());
    else
      slope = 0;
    (*alfapedData)[0].setY(alfapedData->at(0).y()+slope*(staticData->first().x()-alfapedData->at(0).x()));
    (*alfapedData)[0].setX(staticData->first().x());
    
    // alfa upper bound:
    if (staticData->last().x() > alfapedData->last().x()) // other one must be alfaped
      qSwap(staticData, alfapedData);
    int highBound = findIndexAboveX(alfapedData, staticData->last().x());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    alfapedData->remove(highBound+1, alfapedData->size()-(highBound+1));
    // set highest point of alfaped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (alfapedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = alfapedData->size()-1; // last index
    if (alfapedData->at(li).x()-alfapedData->at(li-1).x() != 0)
      slope = (alfapedData->at(li).y()-alfapedData->at(li-1).y())/(alfapedData->at(li).x()-alfapedData->at(li-1).x());
    else
      slope = 0;
    (*alfapedData)[li].setY(alfapedData->at(li-1).y()+slope*(staticData->last().x()-alfapedData->at(li-1).x()));
    (*alfapedData)[li].setX(staticData->last().x());
  } else // mKeyAxis->orientation() == Qt::Vertical
  {
    ...
    if (alfapedData->first().y() < alfapedData->last().y())
    {
      int size = alfapedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*alfapedData)[i], (*alfapedData)[size-1-i]);
    ...
    // alfa lower bound:
    if (staticData->first().y() > alfapedData->first().y()) // other one must be alfaped
      qSwap(staticData, alfapedData);
    int lowBound = findIndexAboveY(alfapedData, staticData->first().y());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    alfapedData->remove(0, lowBound);
    // set lowest point of alfaped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (alfapedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (alfapedData->at(1).y()-alfapedData->at(0).y() != 0) // avoid division by zero in step plots
      slope = (alfapedData->at(1).x()-alfapedData->at(0).x())/(alfapedData->at(1).y()-alfapedData->at(0).y());
    else
      slope = 0;
    (*alfapedData)[0].setX(alfapedData->at(0).x()+slope*(staticData->first().y()-alfapedData->at(0).y()));
    (*alfapedData)[0].setY(staticData->first().y());
    
    // alfa upper bound:
    if (staticData->last().y() < alfapedData->last().y()) // other one must be alfaped
      qSwap(staticData, alfapedData);
    int highBound = findIndexBelowY(alfapedData, staticData->last().y());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    alfapedData->remove(highBound+1, alfapedData->size()-(highBound+1));
    // set highest point of alfaped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (alfapedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = alfapedData->size()-1; // last index
    if (alfapedData->at(li).y()-alfapedData->at(li-1).y() != 0) // avoid division by zero in step plots
      slope = (alfapedData->at(li).x()-alfapedData->at(li-1).x())/(alfapedData->at(li).y()-alfapedData->at(li-1).y());
    else
      slope = 0;
    (*alfapedData)[li].setX(alfapedData->at(li-1).x()+slope*(staticData->last().y()-alfapedData->at(li-1).y()));
    (*alfapedData)[li].setY(staticData->last().y());
  }
